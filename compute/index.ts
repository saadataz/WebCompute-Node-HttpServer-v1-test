...
app.get("/staticToCompute", (req: express.Request, res: express.Response) => {
  res
    .status(200)
    .header("x-static-to-compute-header", "static-to-compute-header-value")
    .send("static-to-compute");
});
...
app.get("/ahioToCompute", (req: express.Request, res: express.Response) => {
  res
    .status(200)
    .header("x-ahio-to-compute-header", "ahio-to-compute-header-value")
    .send("ahio-to-compute");
});

app.all("/proxy", (req: express.Request, res: express.Response) => {
  const urlParsed = url.parse(req.url, true);
  const parts = {
    path: req.path,
    queryString: urlParsed.search,
